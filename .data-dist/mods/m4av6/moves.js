	...
	healblock: {
		...
		condition: {
			...
			onTryHeal(damage, target, source, effect) {
				if ((_optionalChain([effect, 'optionalAccess', _24 => _24.id]) === 'zpower') || this.effectData.isZ) return damage;
				return false;
			},
		},
	},
	...
	trick: {
		...
		onHit(target, source, move) {
			...
			if (yourItem) {
				...
				(target ).lostItemForDelibird = myItem;
			}
		},
	},
	moonlight: {
	...
	curse: {
		...
		condition: {
			onStart(pokemon, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _25 => _25.effectType]) === 'Ability') {
