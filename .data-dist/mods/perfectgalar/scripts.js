"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; } const Scripts = {
	...
	newGMaxPower( move ){
		...
		if (!move.basePower) {
			return gmaxPower;
		} else if ( !_optionalChain([move, 'access', _3 => _3.maxMove, 'optionalAccess', _4 => _4.basePower]) ){
			return null;
		} else if (['Fighting', 'Poison'].includes(move.type)) {
			return _optionalChain([move, 'access', _5 => _5.maxMove, 'optionalAccess', _6 => _6.basePower]);
		} else if (['Flying'].includes(move.type)) {
			for ( const i in oldMaxPowers ){
				if ( _optionalChain([move, 'access', _7 => _7.maxMove, 'optionalAccess', _8 => _8.basePower]) === oldMaxPowers[i] ){
			...
			for ( const i in oldMaxPowers ){
				if ( _optionalChain([move, 'access', _9 => _9.maxMove, 'optionalAccess', _10 => _10.basePower]) === oldMaxPowers[i] ){
